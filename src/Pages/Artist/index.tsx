              ...
              <div>
                {loading ? (
                  <div
                    ...
                    style={{ minHeight: type ? "366px" : "448px" }}
                  >
                    <Spinner
                      thickness="4px"
                      speed="0.65s"
                  ...
                  <div
                    className={`grid ${filterCardNum === 4 && "grid-cols-4"} ${filterCardNum === 3 && "grid-cols-3"} ${filterCardNum === 2 && "grid-cols-2"} gap-4 py-5`}
                  >
                    {artists &&
                      artists.map((item, idx) => (
                        <div
                          key={`main-artist-filter-${idx}`}
                          className="w-full"
                        >
                          <Avatar src={item.img} />
                          <div
                            className={`md:text-xl text-md ${themeMode ? "artist-filter-name" : "artist-filter-name-dark"}`}
                            style={{ fontSize: type ? "14px" : "24px" }}
                          >
                            {item.name}
                          </div>
                          <div
                            className={`${themeMode ? "artist-filter-description" : "artist-filter-description-dark"}`}
                            style={{ fontSize: type ? "12px" : "21px" }}
                          >
                            54 Songs
                          </div>
                        </div>
              ...
              <div>
                ...
                >
                  {artist && (
                    <div className="flex items-start w-full">
                      <div className="md:block hidden">
                        <Avatar size="2xl" src={artist.img} />
                      </div>
                      <div className="md:hidden block">
                        <Avatar size="lg" src={artist.img} />
                      </div>
                  ...
                  <div className="md:pr-16">
                    {cardData?.length && (
                      <ArtistsCarousel cardNum={cardNum} cardData={cardData} />
                    )}
                  </div>
                </div>
                {loading ? (
                  ...
                  >
                    {artists &&
                      artists.map((item, idx) => (
                        <div
                          key={`main-artist-${idx}`}
                          className="flex md:items-center items-start w-full"
                        >
                          <div className="md:block hidden">
                          ...
                          <div className="flex flex-col md:ml-4 ml-2 gap-1 md:gap-3">
                            <div
                              className={`flex artist-name md:text-xl text-md ${!themeMode && "title-dark-color"}`}
                            >
                              {item.name}
                            </div>
                            <div
