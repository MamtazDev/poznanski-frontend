...
const menu = ["Newsy", "TV/Radio", "Nasze materiały", "Albumy", "Artyści", "Wydarzenia"]

export const getIconsColor = (themeMode?: boolean) => (themeMode ? "#5A1073" : "#2FC4B2")
  ...
  return (
    <Fragment>
      <div className={`Nav-bar fixed w-full z-50 shadow-xl`}>
        <div className={`Nav-bar-top ${!props.themeMode && "Nav-bar-top-dark"} flex place-items-center justify-center`}>
          <div className="flex justify-between gap-x-1 container py-4 px-4 md:px-6">
            <div className="flex w-full place-items-center">
              <Link to={"/"} className={isSearchExpanded && (isSmallDevice || !isSmallDevice) ? "hidden" : ""}>
                <PoznanskiLogoIcon
              ...
              >
                <MobileMenuIcon className="w-6 h-6" stroke={getIconsColor(props.themeMode)} />
              </button>
            </div>
              ...
              <div className="hidden md:block w-full">
                <div className="flex-2 flex place-items-center gap-x-4 h-full justify-end">
                  <div className="grid grid-cols-4 gap-x-4">
                    <YoutubeIcon className="cursor-pointer" stroke={getIconsColor(props.themeMode)} />
                    <FacebookIcon className="cursor-pointer" stroke={getIconsColor(props.themeMode)} />
                    <InstagramIcon className="cursor-pointer" stroke={getIconsColor(props.themeMode)} />
                    <SummaryIcon className="cursor-pointer" stroke={getIconsColor(props.themeMode)} />
                  </div>
        ...
        {props.type || (
          <div className={`Nav-bar-down ${!props.themeMode && "Nav-bar-down-dark"} space-around`}>
            <div className="flex justify-center gap-10">
              {menu.map((item, idx) => (
                <div
                  key={`nav-btn-${idx}`}
                  className={`Nav-btn ${!props.themeMode && "Nav-btn-dark"} ${
                    item === selectedMenu && (props.themeMode ? "selected-menu" : "selected-menu-dark")
                  }`}
      ...
      <Modal onClose={onClose} size="full" isOpen={openModal}>
        <ModalOverlay />
        <ModalContent className={`container ${!props.themeMode && "back-dark"} justify-between h-screen p-4 md:p-6`}>
          <div className="flex-col h-full">
            <div className="flex justify-between my-4">
              <div className="cursor-pointer">
                <PoznanskiLogoIcon className="rounded-full shadow-2xl" fill={props.themeMode ? "#000" : "#fff"} />
              </div>
              ...
              {menu.map((item, idx) => (
                <div
                  ...
                  className={`Nav-vertical-btn text-center ${
                    item === selectedMenu && (props.themeMode ? "selected-menu" : "text-dark-color")
                  } h-[15%] content-center`}
                  onClick={() => onClick(item)}
                >
                  {item}
